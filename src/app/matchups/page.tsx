...
import type { MergedMatchup } from "@/lib/matchupMerge";
...
// Contenu détaillé (plan de jeu perso + fiche sourcée) d'un adversaire —
// extrait en composant séparé pour être réutilisé à l'identique dans
// l'accordéon mobile ET le panneau de détail de la vue côte à côte iPad/
// desktop ci-dessous, plutôt que de dupliquer ce bloc deux fois.
function MatchupDetailBody({ m }: { m: MergedMatchup }) {
  return (
...
export default function MatchupsPage() {
  ...
  // fois. Initialisée après coup (premier adversaire de la liste) une fois
  // `merged` calculé plus bas.
  ...
  const selected = merged.find((m) => m.slug === selectedSlug) ?? merged[0] ?? null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* PLANS DE JEU DÉTAILLÉS — un plan par adversaire connu. Deux
          présentations selon la largeur d'écran, mêmes données :
          - Mobile/tablette portrait (< lg, ~1024px) : accordéon classique,
            plusieurs fiches dépliables en même temps (comportement inchangé).
          - iPad paysage / desktop (>= lg) : liste + panneau de détail côte à
            côte (un seul adversaire sélectionné à la fois), pour profiter de
            la largeur au lieu d'empiler les fiches verticalement. */}
      <div>
        ...
        {/* --- iPad paysage / desktop : liste + détail côte à côte --- */}
        <div className="hidden lg:grid lg:grid-cols-[320px_1fr] gap-4 items-start">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg border transition-colors ${
                    isSelected ? "bg-panel2 border-emerald" : "border-line hover:border-lineStrong hover:bg-panel2/50"
                  }`}
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    <OpponentLeaderBadge label={m.opponentLabel} size={22} />
                    {diff && <span className={`text-xs font-mono ${diff.color}`}>{diff.icon}</span>}
                    {m.conflicting && <span className="badge badge-gold text-[9px]">⚠</span>}
                  </div>
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    {m.center && <span className="text-[10px] text-gold">{"★".repeat(m.center.trainingPriority)}{"☆".repeat(5 - m.center.trainingPriority)}</span>}
                    {real && (
                      <span className="text-[10px] font-mono text-emerald-bright">
                        {real.wins}V-{real.losses}D ({Math.round((real.wins / real.total) * 100)}%)
                      </span>
                    )}
                  </div>
                </button>
          ...
          <div className="card-tile p-5 sticky top-20">
            {selected ? (
              <>
                <div className="flex items-center gap-3 flex-wrap mb-4 pb-3 border-b border-line">
                  <OpponentLeaderBadge label={selected.opponentLabel} size={30} />
                  {selected.center && (() => {
                    const diff = DIFFICULTY_LABEL[selected.center.difficulty];
                    return <span className={`text-sm font-mono ${diff.color}`}>{diff.icon} {diff.label}</span>;
                  })()}
                </div>
                <MatchupDetailBody m={selected} />
              </>
            ) : (
              <div className="text-sm text-steel/60">Sélectionne un adversaire dans la liste.</div>
            )}
          </div>
        </div>
      </div>
